...
/*   Updated: 2020/04/14 19:56:38 by lcoiffie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_error2(int errornbr)
{
	if (errornbr == 12)
		ft_putstr_fd("Input Error --> Listen..they said only one player\n", 1);
	if (errornbr == 13)
		ft_putstr_fd("Again and over again. Redondant datas in .cub\n", 1);
	if (errornbr == 14)
		ft_putstr_fd("Input Error --> Pfouuu, not a closed map\n", 1);
	if (errornbr == 15)
		ft_putstr_fd("Oh oh ! We don't have correct map data\n", 1);
	if (errornbr == 16)
